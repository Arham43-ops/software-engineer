...
const formatExperiencePeriod = (startDate: string, endDate?: string) => {
    const format = (date: string) => new Date(`${date}T00:00:00`).toLocaleDateString('en-US', {
        month: 'short',
        year: 'numeric',
    });
...
const showcaseMembers = [
    ...portfolioData.experiences.map(exp => ({
        ...
        period: formatExperiencePeriod(exp.startDate, exp.endDate),
        image: {
            'prof-1': '/experience/icat-logo.png',
            'prof-2': '/experience/injala-logo.png',
            'prof-3': '/experience/genuine-bags-storefront.png',
        }[exp.id] || '/about/hero-profile-artwork.png',
        imageFit: exp.id === 'prof-3' ? 'cover' : 'contain',
                                        ...
                                        <div className="flex flex-col gap-4 w-[320px] md:w-[400px] border border-neutral-200 dark:border-neutral-800 p-6 rounded-2xl bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md shadow-xl mt-4">
                                            ...
                                            <div className="w-full mt-4 rounded-xl border border-neutral-200 dark:border-neutral-800 overflow-hidden relative group/card h-32">
                                                <img
                                                    src={member.image}
                                                    alt={member.name}
                                                    className={`w-full h-full ${'imageFit' in member && member.imageFit === 'contain' ? 'object-contain p-4' : 'object-cover'} ${member.id === 'prof-2' ? 'scale-[2.1] mix-blend-screen group-hover/card:scale-[2.2]' : 'group-hover/card:scale-105'} opacity-90 group-hover/card:opacity-100 transition-opacity duration-500`}
                                                />
